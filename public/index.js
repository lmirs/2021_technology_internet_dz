...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите описание по шаблону:\nВАКАНСИЯ: <название вакансии> КОМПАНИЯ: <название компании> ');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новое описание по шаблону:\nВАКАНСИЯ: <название вакансии> КОМПАНИЯ: <название компании> ', oldTaskName);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Вакансия '${taskName}' будет удалена. Продолжить?`)) return;
